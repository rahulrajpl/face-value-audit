...
def build_static_report_html(final, overview, visibility, reputation, marketing, experience, scores, reviews):
    def _kv_table(title, d):
        rows = "".join(
            f"<tr><th>{k}</th><td>{(v if isinstance(v,str) else json.dumps(v)) or '—'}</td></tr>"
            for k, v in d.items()
        )
    ...
    <style>
      ...
      .scores{margin:8px 0 16px}
      .chip{display:inline-block;margin:0 12px 8px 0;padding:6px 10px;background:#f5f5f7;border-radius:8px}
      a{color:#0b57d0;text-decoration:none} a:hover{text-decoration:underline}
      section{margin-bottom:16px}
    </style>
    """

    title = final.get("practice_name") or "Smile Audit"
    addr  = final.get("address") or "—"
    maps  = final.get("maps_link")
    addr_html = f'<a href="{maps}" target="_blank" rel="noopener">{addr}</a>' if (maps and final.get("address")) else addr

    header = f"""
      <h1>Smile Audit – {title}</h1>
      <div><strong>Website:</strong> {final.get('website','—')} &nbsp;|&nbsp;
           <strong>Email:</strong> {final.get('email','—')} &nbsp;|&nbsp;
           <strong>Phone:</strong> {final.get('phone','—')}<br>
           <strong>Address:</strong> {addr_html}
      </div>
      <div class="scores">
        <span class="chip">Overall: {scores['overall']}/100</span>
        <span class="chip">Visibility: {scores['visibility']}/30</span>
        <span class="chip">Reputation: {scores['reputation']}/40</span>
        <span class="chip">Experience: {scores['experience']}/30</span>
      </div>
    """

    html = f"""<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    <title>Smile Audit – {title}</title>{style}</head><body>
    {header}
    {_kv_table("Overview", overview)}
    {_kv_table("Online Visibility", visibility)}
    {_kv_table("Reputation & Feedback", reputation)}
    {_kv_table("Marketing", marketing)}
    {_kv_table("Patient Experience", experience)}
    {"<section><h2>Recent Google Reviews</h2><ul>" + "".join(f"<li><strong>{(r.get('author_name') or '')}</strong>: { (r.get('text') or '')[:400]}</li>" for r in (reviews or [])[:10]) + "</ul></section>" if reviews else ""}
    </body></html>"""
    return html
